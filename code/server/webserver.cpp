...
WebServer::WebServer(
    ...
    bool openLog, int logLevel, int logQueSize) : port(port), openLinger(optLinger), timeoutMs(timeoutMs), shutdown(false),
                                                  timer(new HeapTimer()), epoller(new Epoller())
{
    ...
    if (openLog)
    {
        ...
        else
        {
            ...
            LOG_INFO("srcDir: %s", Http_Connect::srcDir);
...
void WebServer::initEventMode(int trigMode)
{
    ...
    //用&判断连接套接字是否为ET触发模式
    Http_Connect::isET = (connEvent & EPOLLET);
}
...
void WebServer::closeConnect(Http_Connect *client)
{
    assert(client);
...
{
    assert(fd > 0);
    // users是哈希表，套接字是键，Http_Connect对象是值
    //将fd和连接地址传入,初始化Http_Connect对象，用client表示
...
void WebServer::dealListen()
{
    ...
    do
    {
        ...
        if (fd <= 0)
            return;
        if (Http_Connect::userCnt >= MAX_FD)
        {
...
void WebServer::dealRead(Http_Connect *client)
{
    assert(client);
    extentTime(client);
...
void WebServer::dealWrite(Http_Connect *client)
{
    assert(client);
    extentTime(client);
    //非静态成员函数需要传递this指针作为第一个参数
...
//重置计时器
void WebServer::extentTime(Http_Connect *client)
{
    assert(client);
    if (timeoutMs > 0)
...
/* 读函数：先接收再处理 */
void WebServer::onRead(Http_Connect *client)
{
    assert(client);
    int ret = -1;
    int readErrno = 0;
...
void WebServer::onWrite(Http_Connect *client)
{
    assert(client);
    int ret = -1;
    int writeErrno = 0;
